...
function siteIcon(blobUrl, color, label) {
  const labelHtml = label
    ...
    : '';
  const pinClass = `cc-tactical-map-pin${label ? ' has-label' : ''}`;
  // Etiqueta completa (sin ellipsis): la caja crece y envuelve hasta 4 líneas.
  const labelLines = label ? Math.min(4, Math.ceil(label.length / 22)) : 0;
  const labelH = labelLines ? 3 + labelLines * 14 + 6 : 0;
  ...
  if (blobUrl) {
    return L.divIcon({
      ...
      iconSize: [iconW, 36 + labelH],
      // Centro del círculo (36px), no de la etiqueta.
      iconAnchor: [anchorX, 18],
      popupAnchor: [0, -20],
    });
  }
...
/**
 * Capa de sitios en mapas de despacho.
 * @param {object[]} sites
