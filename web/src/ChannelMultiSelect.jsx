  ...
  return (
    <div className="channel-col-mode">
      {hasPanelSelect ? (
        ...
        <span className="channel-col-mode-title">{label}</span>
      ) : null}
      <select
        value={value}
    ...
    <div className="channel-col-tools">
      <div className="channel-col-tools-row channel-col-tools-main">
        ...
        <span
          ...
          title="Ascendente/Descendente ordena la lista. Marcar/Desmarcar alterna todos los checks (solo Múltiple). Buscar filtra por nombre."
        >
          ?
        </span>
      </div>
  ...
  function renderGroupRow(g, column) {
    ...
    return (
      ...
      >
        ...
        <span className="channel-dual-movers">
          <button
            type="button"
            className={`channel-dual-move${isFirst ? ' is-edge' : ''}`}
            aria-label={`Subir ${g.name}`}
            disabled={isFirst}
            title={isFirst ? 'Ya es el primero' : 'Subir'}
            onClick={() => moveRelative(g.id, -1)}
          >
            ↑
          </button>
          <button
            type="button"
            className={`channel-dual-move${isLast ? ' is-edge' : ''}`}
            aria-label={`Bajar ${g.name}`}
            disabled={isLast}
            title={isLast ? 'Ya es el último' : 'Bajar'}
            onClick={() => moveRelative(g.id, 1)}
          >
            ↓
          </button>
  ...
  function columnConfig(column) {
    ...
    if (column === 'talk') {
      return {
        ...
        hint:
          talkMode === 'multiple' && talkIds.length > 1
            ? `Al pulsar PTT transmites a los ${talkIds.length} canales marcados.`
            : 'Hablar marca automáticamente Escuchar. Arrastra o usa ↑↓ para ordenar.',
      };
    }
    if (column === 'video') {
      return {
        ...
        hint:
          videoMode === 'multiple' && videoIds.length > 1
            ? `Videollamada unifica a los miembros de los ${videoIds.length} canales (sin duplicar).`
            : 'Individual = un canal. Múltiple = una sola llamada con todos los marcados.',
      };
    }
    return {
      mode: alertMode,
      ...
      hint:
        alertMode === 'multiple' && alertIds.length > 1
          ? `Enviar alerta a ${alertIds.length} canales · 1 aviso por persona.`
          : 'Destino de Enviar alerta. Múltiple = sin duplicar personas.',
    };
  }
        ...
        <ul className="channel-dual-list">
          {cfg.mode === 'individual' && (
            <li className={`channel-dual-row none${cfg.selectedIds.length === 0 ? ' on' : ''}`}>
              ...
              <span className="channel-dual-movers is-spacer" aria-hidden="true" />
            </li>
          )}
        ...
        <div className="channel-layout-bar">
          <label className="channel-layout-mode">
            ...
            >
              ...
              <option value="select">Select en encabezado</option>
            </select>
          </label>
        </div>
      ) : null}
